...
public class LevelBuilder
{
    ...
    public void buildConcreteBoundaryWalls(GameScene scene, double worldWidth, double worldHeight)
    {
        ...
        if (leftWidth > 0)
            scene.addGameObject(new ConcreteWall(new Vector2D(leftWidth / 2.0, worldHeight / 2.0), leftWidth, worldHeight));

        if (rightWidth > 0)
            scene.addGameObject(new ConcreteWall(new Vector2D(maxX + rightWidth / 2.0, worldHeight / 2.0), rightWidth, worldHeight));

        if (topHeight > 0 && innerWidth > 0)
            scene.addGameObject(new ConcreteWall(new Vector2D(minX + innerWidth / 2.0, topHeight / 2.0), innerWidth, topHeight));

        if (bottomHeight > 0 && innerWidth > 0)
            scene.addGameObject(new ConcreteWall(new Vector2D(minX + innerWidth / 2.0, maxY + bottomHeight / 2.0), innerWidth, bottomHeight));
    }
    ...
    public void buildLevel1InternalConcreteWalls(GameScene scene, Level1CollisionMask level1CollisionMask, double worldWidth, double worldHeight)
    {
        ...
        for (int row = 0; row < rows; row++)
        {
            ...
            {
                blocked[row][col] = isBlockedTerrainCell(level1CollisionMask, col, row, CELL_SIZE, worldWidth, worldHeight);
            }
        }
